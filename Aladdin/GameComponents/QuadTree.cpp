...
void QuadTree::removeEntity(Entity *entity)
{
	...
	if (index == -1)
	{
		if (mListEntity.size() == 1)
		{
			mListEntity.clear();
			return;
		}
		//neu trong list co nhieu hon 1 entity thi ta se xoa entity bang cach truyen vao index cua entity do trong list
		else
		{
			for (size_t i = 0; i < mListEntity.size(); i++)
			{
/*				if (entity->Tag == Entity::EntityTypes::Guard)
				{
